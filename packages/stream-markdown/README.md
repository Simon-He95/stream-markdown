...
- updateCodeTokensIncremental(container, highlighter, code, opts): Incrementally update DOM with tokens; falls back to full render on divergence. Accepts optional `tokenLines` to reuse precomputed tokens (e.g., from shiki-stream) instead of re-tokenizing.
