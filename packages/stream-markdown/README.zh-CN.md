...
- `updateCodeTokensIncremental(container, highlighter, code, opts)`：基于 tokens 的增量更新；若发现早前行出现分歧会回退到全量渲染。`opts.compareMode` 可选 `'signature'`（默认）或 `'innerHTML'`，也支持传入 `tokenLines` 以复用外部缓存的 tokens（例如 @shikijs/stream），避免重复高亮。`opts.tokenCache` / `opts.tokenCacheMaxEntries` 控制 token 缓存；`opts.htmlCache` / `opts.htmlCacheMaxEntries` 控制完整 HTML 缓存。
...
- `createShikiStreamCachedRenderer(container, opts)`：与 `createShikiStreamRenderer` 接口一致，但底层复用 @shikijs/stream 的 grammarState，高亮追加内容时无需重新遍历已有前缀；可通过 `useGrammarState: false` 关闭。
